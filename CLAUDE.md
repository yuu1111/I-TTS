...
### 言語対応
- **日本語優先**: このプロジェクトは日本のコミュニティ向けのため、ユーザーとの対話は原則として日本語で行う
- **技術用語**: 一般的な技術用語（API、HTTP、JSONなど）はそのまま使用可能
- **エラーメッセージ**: ユーザー向けのエラーメッセージや説明は日本語で提供
- **コード内コメント**: コードやコメントは英語を基本とするが、複雑な処理の説明は日本語コメントも許可
